...
int main()
{
	...
	//绑定顶点数组对象，随后的顶点属性都会保存到这个VAO里面 直到解绑为止（这样的好处是如果有多个VBO的时候不用频繁的操作）
	...
	//把用户定义的数据传输到当前绑定的显存缓冲区中。  最后一个参数是告诉显卡这些顶点的属性是不变的还是容易改变的，显卡会通过该属性决定这些顶点数据的储存位置
